...
class Tokenizer {
    ...
    constructor(
        ...
        const handler = {
            set(target: any, property: string, value: any): boolean {
                const properties = ['_tokens', '_rules', '_cursor', '_line', '_column']
                ...
                if (properties.includes(property)) {
                    ...
                    // push the operation object to the history array
                    self._history.push(operation)
                }
    ...
    private updatePosition(token: Token, line_no: number): void {
        ...
        this._column = (this._line !== line_no) ? 0 : this._column + token.value.length
